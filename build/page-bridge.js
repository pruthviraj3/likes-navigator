(function () {
    ...
    async function handleCommand(message) {
        ...
        try {
            ...
            if (message.action === "EXTRACT_RANGE") {
                ...
                const result = await extractLikedPostsForRange(message.payload || {}, (progress) => emitProgress(requestId, progress));
    ...
    function matchesLikedEndpoint(urlString) {
        try {
            ...
            return (url.pathname === "/async/wbloks/fetch/" &&
    ...
    function publishCapturedPayload({ requestUrl, requestBody, rawResponseText, transport }) {
        ...
        const mediaItems = extractedItems.length > 0 ? extractedItems : extractMediaItemsFromParsedPayload(payload);
        window.postMessage({
    ...
    function extractMediaItems(rawText) {
        const items = [];
        const seen = new Set();
        const blockRegex = /\(bk\.action\.map\.Make,\s*\(bk\.action\.array\.Make,\s*"media_id",\s*"media_code",\s*"media_product_type",\s*"media_type",\s*"media_image_url",\s*"location_name",\s*"icon",\s*"margin_right"\),\s*\(bk\.action\.array\.Make,\s*"([^"]*)",\s*"([^"]*)",\s*"([^"]*)",\s*\(bk\.action\.i32\.Const,\s*(\d+)\),\s*"((?:\\.|[^"\\])*)",\s*"((?:\\.|[^"\\])*)",\s*"([^"]*)",\s*"([^"]*)"\)\)/g;
        const fallbackTupleRegex = /\(bk\.action\.array\.Make,\s*"([^"]+)",\s*"([^"]+)",\s*"([^"]+)",\s*\(bk\.action\.i32\.Const,\s*(\d+)\),\s*"((?:\\.|[^"\\])*)",\s*"((?:\\.|[^"\\])*)",\s*"([^"]*)",\s*"([^"]*)"\)/g;
    ...
    function matchToMediaItem(match) {
        const [, mediaId, mediaCode, mediaProductType, mediaTypeRaw, mediaImageUrlRaw, locationNameRaw, icon] = match;
    ...
    function extractNextPageInfo(rawText) {
        return (extractNextPageInfoFromManifest(rawText) ||
    ...
    function extractNextPageInfoFromManifest(rawText) {
        ...
        for (const marker of markers) {
            ...
            const labelArraySuffix = argsStart !== -1
            ...
            if (!source.startsWith("(bk.action.array.Make, ", valueArrayStart)) {
                continue;
            }
    ...
    function extractNextPageInfoDirect(rawText) {
        const regex = /AsyncActionWithDataManifest,\s*\\?"com\.instagram\.privacy\.activity_center\.liked_next\\?".*?\(bk\.action\.array\.Make,\s*\\?"(\d+)\\?",\s*\\?"((?:\\.|[^"\\])*ActivityCenterParams(?:\\.|[^"\\])*)\\?",\s*\\?"(QVF[A-Za-z0-9+/=_-]+)\\?",\s*\\?"(\d+)\\?",\s*\\?"(\d+)\\?"/s;
    ...
    function extractNextPageInfoHeuristic(rawText) {
        ...
        for (const marker of candidates) {
            ...
            const quoted = [...chunk.matchAll(/"((?:\\.|[^"\\])*)"/g)].map((match) => unescapeBloksString(match[1]));
            const cursors = quoted.filter((value) => /^QVF[A-Za-z0-9+/=_-]{20,}$/.test(value));
            const numericIds = quoted.filter((value) => /^\d{8,}$/.test(value));
            const activityCenterParams = quoted.find((value) => value.includes("ActivityCenterParams"));
    ...
    function buildRefreshParams({ rangeStartUnix, rangeEndUnix, sort, refreshTemplate }) {
        ...
        return {
            ...
            main_attribute_order_state_value: buildSortStateValue(sort, base.main_attribute_order_state_value),
            ...
            main_filter_to_visible_on_facebook_value: Boolean(base.main_filter_to_visible_on_facebook_value ?? false),
            ...
            main_account_history_events_state_value: stringOrEmpty(base.main_account_history_events_state_value),
            entrypoint: stringOrEmpty(base.entrypoint),
            shared_user_id: base.shared_user_id == null ? "" : String(base.shared_user_id),
            main_filter_to_visible_from_facebook_value: Boolean(base.main_filter_to_visible_from_facebook_value ?? false)
        };
    }
    function buildSortStateValue(sort, existingValue) {
        ...
        return {
            "py/reduce": [
                {
                    "py/type": "bloks.apps.instagram.privacy.activity_center.general_constants.ActivityCenterSortAndFilterConstants"
                },
                {
                    "py/tuple": [sort]
                }
    ...
    function buildNextParams(nextPageInfo, paginationState) {
        const parsedManifestParams = parseActivityCenterParamsJson(String(nextPageInfo.activity_center_params || ""));
        ...
        merged.content_container_id = pickNumericId(parsedManifestParams?.content_container_id, latestKnownState.content_container_id, stableFilterState.content_container_id, containerId);
        merged.content_element_id = pickNumericId(parsedManifestParams?.content_element_id, latestKnownState.content_element_id, stableFilterState.content_element_id, elementId);
        merged.content_spinner_id = pickNumericId(parsedManifestParams?.content_spinner_id, latestKnownState.content_spinner_id, stableFilterState.content_spinner_id);
    ...
    async function extractLikedPostsForRange(opts, onProgress = null) {
        ...
        try {
            ...
            const sort = opts.sort === "newest_to_oldest" ? "newest_to_oldest" : "oldest_to_newest";
            ...
            const derivedRange = startDate && endDate
            ...
            if (!initialTemplate) {
                throw new Error("No Instagram liked-request template is available yet. Open the Likes activity page and wait for it to load once.");
            }
    ...
    function resolveTemplate(appId, fallbackTemplate) {
        ...
        if (appId === LIKED_REFRESH_APP_ID) {
            return (deriveTemplate(bridgeState.templatesByAppId.get(LIKED_MEDIA_SCREEN_APP_ID), appId) ||
                deriveTemplate(bridgeState.templatesByAppId.get(LIKED_NEXT_APP_ID), appId));
        }
        if (appId === LIKED_NEXT_APP_ID) {
            return (deriveTemplate(bridgeState.templatesByAppId.get(LIKED_REFRESH_APP_ID), appId) ||
    ...
    async function postInitialExtractorRequest({ initialTemplate, paramsPayload, reqToken }) {
        try {
            ...
            throw new Error(`Initial liked-post request failed via ${initialTemplate?.appId || "unknown"}. ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    ...
    function validateRefreshParams(params) {
        if (!Number.isFinite(Number(params.content_container_id)) ||
            !Number.isFinite(Number(params.content_element_id)) ||
            !Number.isFinite(Number(params.content_spinner_id))) {
            throw new Error("No valid liked_refresh template is captured yet. Open the Likes page, change or apply filters once, and try again.");
        }
    }
    function validateNextParams(params) {
        ...
        try {
            ...
            throw new Error(`Instagram liked_next activity_center_params is not valid JSON. Preview: ${params.activity_center_params.slice(0, 160)}`);
        }
        if (parsed["py/object"] !==
